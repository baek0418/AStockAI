...
from astock_core.analysis.analysis_evidence import build_report_evidence
...
def _format_amount(value):
    return f"¥{value:,.0f}" if _is_number(value) else "数据不足"
...
def _format_percent(value):
    return f"{value:.2f}%" if _is_number(value) else "数据不足"


def create_reader_portfolio_section(portfolio_context):
    """把日报从关注列表提升到个人持仓的事实核对，不给出交易指令。"""
    context = portfolio_context or {}
    summary = context.get("摘要")
    lines = ["## 我的持仓：今天先核对什么", ""]
    if not isinstance(summary, dict):
        return "\n".join(lines + ["- " + str(context.get("状态", "未录入本地持仓。"))])

    count = summary.get("持仓数量", 0)
    lines.append(
        f"- 已录入 {count} 只股票；已匹配本地收盘的市值 {_format_amount(summary.get('已报价持仓市值'))}，"
        f"相对录入成本的浮盈亏 {_format_amount(summary.get('已报价浮盈亏'))}。"
    )
    risk_weight = context.get("风险市值占比")
    if _is_number(risk_weight):
        lines.append(
            f"- 其中 {context.get('风险持仓数', 0)} 只持仓当前被规则标为偏弱或风险，"
            f"占已报价持仓市值 {risk_weight:.2f}%；这是今日优先核对范围，不是卖出指令。"
        )
    dates = context.get("最新报价日期", [])
    if dates:
        lines.append("- 本地收盘日期：" + "、".join(dates) + "；不是实时盘中价格。")
    lines.append("- " + str(context.get("状态", "持仓资料不足。")))

    review = context.get("核对清单", [])
    if review:
        lines.extend(["", "**优先核对**"])
        for item in review[:3]:
            weight = _format_percent(item.get("持仓占比"))
            lines.append(
                f"- {item.get('股票名称', '未知股票')}：持仓占比 {weight}；"
                f"{item.get('今日待核对', '资料不足')} 下一观察点：{item.get('下一观察点', '资料不足')}"
            )
    return "\n".join(lines)
...
def _reader_financial_attention(stock):
    """只提示报表内已出现的结构性分歧，避免从单一财务指标推出结论。"""
    fundamental = stock.get("基本面研究证据", {})
    metrics = fundamental.get("指标", {}) if fundamental.get("数据状态") == "可用" else {}
    revenue = metrics.get("营业总收入同比增长")
    profit = metrics.get("归母净利润同比增长")
    debt = metrics.get("资产负债率")
    if _is_number(revenue) and _is_number(profit) and abs(profit - revenue) >= 20:
        return (
            f"营收与归母净利润增速相差 {abs(profit - revenue):.2f} 个百分点；"
            "下一次应结合定期报告核对利润变动来源，不能只看单个增速。"
        )
    if _is_number(debt) and debt >= 70:
        return f"资产负债率为 {debt:.2f}%；应结合定期报告继续核对偿债与现金流信息。"
    return None


def _reader_price_structure(stock):
    """翻译已保存的多周期收益、波动和成交事实，不把它们当作预测。"""
    structure = stock.get("当前量化证据", {}).get("价格结构", {})
    if not isinstance(structure, dict):
        return "当日快照未包含多周期价格结构资料。"
    returns = []
    for days, key in ((5, "5日涨跌"), (10, "10日涨跌"), (20, "20日涨跌")):
        value = structure.get(key)
        if _is_number(value):
            returns.append(f"近 {days} 日 {_format_signed_percent(value * 100)}")
    parts = ["；".join(returns)] if returns else []
    volatility = structure.get("20日波动率")
    volume = structure.get("相对5日成交量")
    if _is_number(volatility):
        parts.append(f"近 20 日日度波动 {volatility * 100:.2f}%")
    if _is_number(volume):
        parts.append(f"成交量为近 5 日均量 {volume:.2f} 倍")
    twenty_day = structure.get("20日涨跌")
    if _is_number(twenty_day) and _is_number(volume):
        if twenty_day > 0 and volume < 0.8:
            parts.append("近期价格上行但成交低于近 5 日常态，延续性仍需确认")
        elif twenty_day < 0 and volume > 1.2:
            parts.append("近期价格走弱且成交放大，需留意波动是否继续扩大")
    return "；".join(parts) if parts else "当日快照未包含多周期收益、波动或成交资料。"
...
def create_reader_focus_section(stocks):
    ...
    for stock in focus_stocks:
        ...
        financial_attention = _reader_financial_attention(stock)
        if financial_attention:
            lines.append(f"- 财务核对点：{financial_attention}")
        lines.append(f"- 价格结构：{_reader_price_structure(stock)}。")
...
def create_evidence_report_content(evidence, ai_summary):
    ...
    email_sections = [
        ...
        create_reader_portfolio_section(evidence.get("持仓核对")),
